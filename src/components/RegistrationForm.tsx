        ...
        {divisions.length > 0 ? (
          <div className="space-y-4">
            {divisions.map((division) => {
              ...
              return (
                <div key={division.id} className="border border-slate-200 rounded-xl">
                  <label className={`flex items-center justify-between p-4 rounded-t-xl ${isSelected ? 'bg-indigo-50' : 'bg-slate-50 hover:bg-slate-100 rounded-b-xl'}`}>
                    ...
                    <div className="p-4 bg-white border-t border-slate-200 rounded-b-xl">
                      ...
                      <div className="flex gap-4 mb-3">
                        <label className="flex items-center gap-2 cursor-pointer">
                          ...
                          />
                          <span className="text-sm text-slate-600 font-medium">Select existing player</span>
                        </label>
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            type="radio"
                            ...
                            className="text-indigo-600 focus:ring-indigo-500"
                          />
                          <span className="text-sm text-slate-600 font-medium">Invite by email</span>
                        </label>
                      </div>
                      ...
                      {partner?.type === 'existing' ? (
                        <div className="relative">
                          <div className="relative">
                            <input
                              ...
                              className="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none shadow-sm"
                            />
                            {partnerSearch[division.id] && (
                              <button 
                                type="button"
                                onClick={() => {
                                  setPartnerSearch(prev => ({ ...prev, [division.id]: '' }))
                                  setDoublesPartners(prev => ({ ...prev, [division.id]: { type: 'existing' } }))
                                }}
                          ...
                          {showPartnerDropdown[division.id] && (
                            <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-xl max-h-60 overflow-auto">
                              {filteredPartners(division.id).length > 0 ? (
                                <div className="py-1">
                                  {filteredPartners(division.id).map(p => (
                                    <button
                                      ...
                                      className="w-full text-left px-4 py-2.5 hover:bg-indigo-50 transition-colors group"
                                    >
                                ...
                                <div className="px-4 py-4 text-sm text-slate-500 bg-slate-50 italic">
                                  No eligible partners found matching criteria.
                                </div>
                              )}
                            </div>
                          )}
                          {partner?.playerId && (
                            <div className="flex items-center gap-1 mt-2 px-1">
                              ...
                              <p className="text-xs font-bold text-emerald-600 uppercase tracking-tight">Partner: {partnerSearch[division.id]}</p>
                            </div>
                          )}
                        </div>
                      ) : (
                        <input
                          ...
                          className="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm text-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none"
                        />
                      )}
                    </div>
                  )}
