...
function FeatureCard({ icon, title, description, delay }: { icon: string; title: string; description: string; delay: number }) {
  return (
    <AnimatedSection delay={delay}>
      <div className="group relative p-8 rounded-3xl bg-white/80 backdrop-blur-sm border border-slate-200/50 hover:border-indigo-500/30 transition-all duration-500 hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-1">
        <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-indigo-500/5 via-transparent to-orange-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        <div className="relative">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center text-2xl mb-5 shadow-lg shadow-indigo-500/25 group-hover:scale-110 transition-transform duration-300">
            {icon}
          </div>
...
function ChapterCard({ name, location, status, href }: { name: string; location: string; status: "active" | "coming"; href: string }) {
      ...
      <main className="pt-28">
        ...
        <section id="ratings" className="px-6 py-24 bg-white">
          <div className="max-w-5xl mx-auto">
            ...
            {/* Match Result Card - moved from hero */}
        ...
        {/* Find Your Chapter - moved after TFR */}
